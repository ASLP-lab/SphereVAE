...
def load_checkpoint_weight_only(checkpoint_path, model):
    """Load only model parameters from either checkpoint format.

    Supported files contain either a ``model`` field or are a raw state dict.
    Optimizer, scheduler, and training metadata are intentionally ignored.
    """
    print("start load weight-only ", checkpoint_path)
    assert os.path.isfile(checkpoint_path)
    checkpoint = torch.load(checkpoint_path, map_location="cpu")
    saved_state_dict = checkpoint.get("model", checkpoint)

    target = model.module if hasattr(model, "module") else model
    current_state_dict = target.state_dict()
    new_state_dict = {}
